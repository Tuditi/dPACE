...
contract dPACE{
    ...
    //Constructor specifies addresses of previously deployed contracts on which dPACE depends
    constructor(
        PKI _PKI,
    ...
    modifier unusedPPC(bytes32 _r, bytes32 _s, uint8 _v){
        require(!PPCUsed[uint(keccak256(abi.encode(_v,_r,_s)))],"Privacy-preserving credential already used!");
        _;
    }
    /*Phase 1: renter and car are deployed as entities on the blockchain
    Inputs: - Privacy-preserving credential -> generated by registration service as in dPACE
            - (r,s,v) signature of the registration service on the privacy-preserving credential 
            - Verify_deployRenterproof is the zk-proof generated by zkay in scenario.js. 
            - genParam: the encrypted balance
    Output: - Event with the credential and the address    
    */
        ...
        public renterAvailable() unusedKeyImage(_signature[0])
    {
        ...
        require(ringSignature.RingVerify(_message.content, _signature), 'Ring Signature is invalid');
    ...
    function isSignatureValid(address _address, Message memory _msg, uint8 _v, bytes32 _r, bytes32 _s) private pure returns(bool) {
