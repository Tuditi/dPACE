...
contract SignatureOrig {
    ...
    {
        return (uint256(keccak256(abi.encode(message.destination, message.value, left[0], left[1], right[0], right[1]))) % N);
    }
    ...
    {
        bool onCurve;
        h[0] = uint256(keccak256(abi.encode(p[0], p[1]))) % N;
